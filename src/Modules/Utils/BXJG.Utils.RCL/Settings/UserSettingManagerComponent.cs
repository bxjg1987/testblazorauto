...
namespace BXJG.Utils.RCL.Settings
{
    ...
    public abstract class UserSettingManagerComponent<TGroupDto, TDto, TEditDto> : Components.BaseComponent
        ...
        where TEditDto : SettingEditDto,new()
    {
        ...
        protected virtual async Task LoadData()
        {
            ...
            {   ///api/services/app/Settings/GetAllGroups
                DataSource = await HttpClient.Post<List<TGroupDto>>("UserSettings", action: "GetAllGroups", default);
            }
            finally
            {
                isLoading = false;
        ...
        protected virtual async Task LoadCurrData()
        {
            ...
            try
            {
                CurrItems = await HttpClient.Post<List<TDto>>("UserSettings", action: "GetSettingByGroup", new { id = currGroup.Name }, default);
            }
            finally
            {
                isLoading = false;
        ...
        protected virtual async Task Save()
        {
            ...
            try
            {
                await HttpClient.Post("UserSettings", action: "Update", CurrItems.Select(x => new TEditDto
                {
                    Name = x.Name,
                    Value = x.Value
                }), default);
    ...
    /// 默认的设置管理组件
    /// </summary>
    public class UserSettingManagerComponent : UserSettingManagerComponent<SettingDefinitionGroupDto,
        SettingDto,
        SettingEditDto>
    {
    }
