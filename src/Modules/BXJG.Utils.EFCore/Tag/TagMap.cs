...
namespace BXJG.Utils.EFCore.Tag
{
    public class TagMap : IEntityTypeConfiguration<TagEntity>
    {
        public void Configure(EntityTypeBuilder<TagEntity> builder)
        {
            ...
            builder.Property(c => c.PropertyName).IsUnicode(false).HasMaxLength(BXJGUtilsConsts.AttachmentEntityPropertyNameMaxLength).HasComment("属性名，可空 比如工单：字段A表示要处理的问题相关tag，字段B表示处理完成时拍摄的tag，它们都使用tag表，当通过此字段来表示关联的不同的属性"); ;
