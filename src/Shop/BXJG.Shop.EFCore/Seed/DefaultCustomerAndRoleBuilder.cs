...
namespace BXJG.Shop.Seed
{
    ...
    public class DefaultCustomerAndRoleBuilder<TTenant, TRole, TUser, TSelf>
        ...
        where TSelf : AbpZeroDbContext<TTenant, TRole, TUser, TSelf>
    {
        ...
        DbSet<TRole> roles;
        public DefaultCustomerAndRoleBuilder(TSelf context, int tenantId)
        {
            _context = context;
            _tenantId = tenantId;
        ...
        {
            //初始化顾客的默认角色
            var role = _context.Roles.IgnoreQueryFilters().FirstOrDefault(r => r.TenantId == _tenantId && r.Name == CoreConsts.CustomerRoleName);
            if (role == null)
            {
                var sdf = new TRole();
                sdf.TenantId = _tenantId;
                sdf.Name = CoreConsts.CustomerRoleName;
                sdf.DisplayName = CoreConsts.CustomerRoleName;
