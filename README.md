...
# 简介
[abp10.x(一代)](https://aspnetboilerplate.com/)是基于asp.net core 10.x的快速开发框架，包含：用户角色权限、多租户、多语言、模块化、日志、后台任务、缓存、对象映射、通知、等等大部分项目都会用到的特征，完整特征请查看它的官方文档。本项目是基于它搭建，并增加了更多功能。
> 你应该先熟悉[abp10.x(一代)](https://aspnetboilerplate.com/)，因为本文档不会包含官方文档已存在的内容。

**此项目 = [abp10.x(一代)](https://aspnetboilerplate.com/) + 通用abp模块 + 帮助库 + blazor web app （auto模式）** 
...
**blazor** 
[abp10.x(一代)](https://aspnetboilerplate.com/)默认是不支持blazor的，但本项目对blazor web app （auto模式）做了集成，且默认使用[AntBlazor](https://ant-design-blazor.gitee.io/zh-CN/components/overview)，你可以修改代码替换成其它ui框架。

# 特征
1. [abp10.x(一代)](https://aspnetboilerplate.com/)的所有特征
1. blazor web app （auto模式）集成，默认使用[AntBlazor](https://ant-design-blazor.gitee.io/zh-CN/components/overview)
...
1. 增强抽象的CrudAppService，增加批量操作；允许更细粒度的方法重写
1. 抽象的CRUD blazor组件；抽象的select组件；
1. 代码生成器
# 资源
在线demo地址：https://zlj.cqyuzuji.com:19911  \
...
# 项目结构
![输入图片说明](docs/images/xiangmujiegou.png)

分为公共库和主项目库，通常我们将公共库发布为nuget包，然后被主项目引用。
主项目就是具体项目，来个新项目时需要复制一份，多个具体项目都是引用相同公共库的nuget包
这样公共库可以一直升级下去。

若使用ZLJ.sln打开解决方案，主项目将以nuget包形式引用公共库。
若使用ZLJWithModules.sln打开解决方案，则所有源码都将列出来。
...
1. **framework下是多个应用共享的库**，它是标准的abp模板项目中的库，然而，一个项目可能有多个应用（如：学校系统中，有教师端、学生端、家长、教务等），我们希望多个应用之间部分逻辑公用，但应用和UI层分开定义，各项目具体含义如下：
    ...
    1. ZLJ.Application.Common.ClientProxy 多应用间的前端共享，后端不引用此库。
...
1.  **host下是托管项目的入口** ，若你有另一个应用，应该按类似的结构建立文件夹和项目。
    1. ZLJ.Web.Host 承载后台管理端的webapi
    1. ZLJ.Web.BlazorAuto 承载后台管理端的UI，它引用ZLJ.Admin.CoreRCL，它是auto模式的server部分。

## tools是工具库
    1. AutoScript一键发布公共库的到nuget服务器的脚本，默认是发布到本地文件夹d:\nugetpackages
    1. CodeGenerator代码生成器，针对简单数据和树形数据的，一键生成crud完整功能（包括ui部分）
