...
在做前端页面时，删除某项之前 需要 两个状态
1. 是否显示删除确认
1. 是否正在删除

要么在dto上加这俩属性，但如果不是blazor项目呢？就显得有点多余。

或者定义个泛型类 class DtoWraper<TDto> 然后把这俩属性定义在这上面
但这样所有返回的dto都被包了一层，很烦。

mudblazor每个行 有个上下文 context对象，它是 CellContext<T>类型的，它上面没有预留  可以添加用户状态的 点，
本来讲这种状态存储在它上面比较合适。


在组件中顶一个 Dictionary<TDto,Dic<string,object>> ,key每行的dto对象，value是此对象的附加属性

数据加载时，清空它，然后重新赋值


回头一想，在dto上定义个 额外属性是可行的，无论是blazor 还是 传统前端，有时候都需要在 对象上关联些数据
