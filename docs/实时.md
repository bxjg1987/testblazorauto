# 20240630更新
目前不考虑客户端通过hub调用服务端功能，因为可以通过接口调用
全局连接仅用来推送通知。
但还是考虑允许这样做

signalR是基于长连接的，要客户端调用服务端方法必须通过连接。
简单的方式是在scope提供字典，key为连接类型，value是连接对象


由于hub类型应该不会太多，通过字符串在调用时不太方便，可以通过appcontainer存储
实现更简单的获取方式。
也可以考虑键控服务
...
BXJG.Utils.RCL中，CommonConnection，它使用scope注册到ioc，这样server模式时可用(注意http请求中的scope和server模式运行时的scope不是同一个)，wasm时自动成了单例。
它负责两个事：
建立signalR连接
处理abp的通知
